...
class EldenEnv(gym.Env):
    ...
    def __init__(self, config):
        ...
        spaces_dict = {  # Observation space (img, prev_actions, state)
            ...
            # Last 10 actions as one hot encoded array
        ...
        # Setting up the reward generator class
        ...
        # Current iteration (number of steps taken in this fight)
        ...
        # Array of the rewards to calculate the average reward of fight
        self.reward_history = []
        ...
        # Desired FPS (not implemented yet)
        ...
        if self.GAME_MODE == "PVE":
            self.walk_to_boss = walkToBoss(
                config["BOSS"])  # Class to walk to the boss
        else:
            self.matchmaking = walkToBoss(99)  # Matchmaking class for PVP mode
            # Lock on class to the player in PVP mode
    ...
    def take_action(self, action):
        ...
        # to prevent spamming heal we only allow it to be pressed every 1.5 seconds
    ...
    def check_for_second_phase(self):
        ...
        self.reward, self.death, self.boss_death, self.duel_won = self.rewardGen.update(
            # perform eldenreward update to get the current status of the boss
            frame, self.first_step)
    ...
    def wait_for_loading_screen(self):
        ...
        # Timer to check the length of the loading screen
        t_check_frozen_start = time.time()
        # We detect the loading screen by reading the text "next" in the bottom left corner of the loading screen.
        ...
        while True:  # We are forever taking a screenshot and checking if it is a loading screen.
            ...
            # If we dont see "next" on the screen we are not in the loading screen [anymore]
            else:
            ...
            # We have been in a loading screen and left it for more than 2.5 seconds
            if have_been_in_loading_screen and (time.time() - t_since_seen_next) > 2.5:
                print('⌛✔️ Left loading screen #1')
                break
            # We have been in a loading screen for 60 seconds. We assume the game is frozen
            elif have_been_in_loading_screen and ((time.time() - t_check_frozen_start) > 60):
                print('⌛❌ Did not leave loading screen #2 (Frozen)')
                # some sort of error handling here...
                # break
            # We have not entered a loading screen for 25 seconds. (return to bonfire and walk to boss) #⚔️ in pvp we use this for waiting for matchmaking
            elif not have_been_in_loading_screen and ((time.time() - t_check_frozen_start) > 20):
                if self.GAME_MODE == "PVE":
    ...
    def check_for_loading_screen(self, frame):
        ...
        # Cutting the frame to the location of the text "next" (bottom left corner)
        next_text_image = frame[1015:1040, 155:205]
        next_text_image = cv2.resize(
            next_text_image, ((205-155)*3, (1040-1015)*3))
        ...
        pytesseract_output = pytesseract.image_to_string(
            # reading text from the image cut out
            mask,  lang='eng', config='--psm 6 --oem 3')
        # Boolean if we see "next" in the text
    ...
    def step(self, action):
        ...
        if self.death:
            ...
            # If the agent has been in control for more than 10 minutes we give up
        ...
        # We resize the frame so the agent dosnt have to deal with a 1920x1080 image (400x225)
        observation = cv2.resize(frame, (MODEL_WIDTH, MODEL_HEIGHT))
        if self.DEBUG_MODE:
            # 🐜 If we are in debug mode we render the frame
        ...
        # Appending the action to the action history
        self.action_history.append(int(action))

        '''FPS LIMITER'''
        t_end = time.time()
        # My CPU (i9-13900k) can run the training at about 2.4SPS (steps per secons)
        ...
        # Current SPS (steps per second)
        current_fps = str(round(((1 / ((t_end - t_start) * 10)) * 10), 1))
        ...
        if not self.done:  # Losts of python string formatting to make the console output look nice
            ...
            print('👣 Iteration: ' + str(self.step_iteration) + '| FPS: ' + current_fps + '| Reward: ' +
                  reward_with_spaces + '| Max Reward: ' + max_reward_with_spaces + '| Action: ' + str(self.action_name))
        else:  # If the game is done (Logging Reward for dying or winning)
            print('👣✔️ Reward: ' + str(self.reward) +
                  '| Max Reward: ' + str(self.max_reward))
    ...
    def reset(self):
        ...
        if self.GAME_MODE == "PVE":
            ...
            # handle the first reset differently (we want to start with the matchmaking, not with losing a duel)
        ...
        frame = self.grab_screen_shot()
        # Reset also returns the first observation for the agent
        observation = cv2.resize(frame, (MODEL_WIDTH, MODEL_HEIGHT))
        spaces_dict = {
            'img': observation,  # The image
            # The last 10 actions (empty)
            'prev_actions': self.oneHotPrevActions(self.action_history),
